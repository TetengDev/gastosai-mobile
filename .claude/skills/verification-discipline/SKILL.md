...
```bash
watch -n60 'ls ~/.maestro/tests/*/ | wc -l'
```
...
`npm install` needs `PACKAGE_TOKEN` in the environment for the pinned contract package.

For end-to-end evidence, `maestro test .maestro/` with the backend up, Metro on :8081 and the app
open in Expo Go. `offline.yaml` and `receipt.yaml` are tagged `manual` and excluded — see
`.maestro/README.md`.
