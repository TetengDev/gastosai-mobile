...
### 6b. Attach a demo recording

**When a feature is added or changed, record it and attach it before the PR opens.**

```bash
./scripts/record-demo.sh <flow> "what changed and what to look at" <LINEAR-ISSUE>
```
...
- **Nothing is attached unless everything is green.** The script runs typecheck, lint and the unit
  tests before it records, and refuses to attach if the demo flow itself fails — the recording is
  kept locally for debugging instead. A clip on the issue therefore *means* the change works, which
  is the only thing that makes it worth opening. A clip of a failure, even a clearly labelled one,
...
- **The clip must narrate itself.** A raw recording is a person tapping a phone: nobody watching
  knows which feature is on trial or what they were meant to notice, and the comment caption does
  not travel with the file once it is downloaded or screenshotted. So a demo flow
...
- Linear rejects uploads at or over 50 MB. A minute of simulator video is a few MB; the full suite
  is not — that is why demos are short and single-purpose.
- **The recording shows real account data and goes to Linear, which keeps it.** Deliberate, never
  automatic: nothing is attached without someone running the script.
- The uploader prints a markdown line for the PR body, so the written evidence and the clip agree.

**Blocker:** a user-visible feature added or changed with no recording attached.
